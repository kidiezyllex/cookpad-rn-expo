...
const SettingItem = ({ item, onPress }: { item: any, onPress?: () => void }) => (
  <Pressable
    onPress={onPress}
    className="flex-row justify-start items-center px-4 py-2 rounded-lg"
    style={{
...
const LogoutButton = () => (
  <Pressable
    className="flex-row justify-center items-center py-1 h-8 rounded-lg opacity-25"
    style={{
      gap: getScaleFactor() * 8,
    }}
    disabled
...
const SettingsSection = ({ section }: { section: typeof settingsSections[0] }) => (
  ...
  >
    <View 
      className="flex-row justify-start items-start px-4"
    ...
    <FlatList
      ...
      scrollEnabled={false}
      ItemSeparatorComponent={() => <View style={{ height: getScaleFactor() * 4 }} />}
    />
    {section.showLogout && <LogoutButton />}
  </View>
);
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      >
        <FlatList
          ...
          keyExtractor={(item) => item.id.toString()}
          scrollEnabled={false}
          ItemSeparatorComponent={() => <View style={{ height: getScaleFactor() * 32 }} />}
          contentContainerStyle={{ gap: getScaleFactor() * 32 }}
        />
      </ScrollView>
    </SafeAreaView>
  );
